...
function handleEvent(ev) {
  ...
  switch (ev.type) {
    ...
    case 'started': lastDryRun = ev.dryRun; log(ev.dryRun ? 'dry run, nothing will be deleted' : 'starting...'); break;
    ...
    case 'done':
      // done/stopped carry stats at the top level (emit('done', stats)), not under
      // a nested "stats" key like the per-message events - so read ev.deleted.
      finish(lastDryRun
        ? ('scan done, ' + (ev.deleted || 0) + ' would be deleted')
        : ('done, ' + (ev.deleted || 0) + ' gone'));
      break;
...
async function gatherOptions() {
  ...
  return {
    ...
    includeNsfw: true,   // pinned + age-restricted are included by default
    includePinned: true,
    limit: Math.max(0, parseInt($('limit').value, 10) || 0), // only the most recent N (0 = all)
