...
(function () {
  ...
  async function search(o, offset) {
    ...
    while (true) {
      ...
      // real delete run and goes through the same 429 backoff.
  ...
  async function run(rawOpts) {
    ...
    // Does this message pass all the on-device filters? Content / link / file /
    // embed are checked against the real message here, not via Discord's flaky
    // word-based search (which misses substrings like "txt" inside "43534txt7798").
    ...
    try {
      ...
      // Dry run: collect once, then walk the list reporting each "would delete".
      if (o.dryRun) {
        ...
        for (const m of queue) {
          ...
          emit('would-delete', { id: m.id, channel: m.channel_id, stats });
      ...
      while (!stopFlag) {
        ...
        for (const m of queue) {
          ...
          // Long session cooldown: after N real deletes, rest for a while so the
          // per-hour volume stays in human territory.
